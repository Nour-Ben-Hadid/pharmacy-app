...
const PatientDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPatientDashboardData = async () => {
      try {
        ...
        const prescriptionsResponse = await apiClient.get('/prescriptions/patient');
  ...
  const handleSaveProfile = (updatedPatient: Patient) => {
  ...
  // Calculate days since last prescription
                    ...
                    <TableBody>
                      {recentPrescriptions.slice(0, 5).map((prescription) => (
                        <TableRow 
                          ...
                          hover
                          sx={{ cursor: 'pointer' }}
                          onClick={() => navigate(`/patient-dashboard/prescriptions/${prescription.id}`)}
                        >
